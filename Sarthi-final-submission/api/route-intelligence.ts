export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const hasStartCoordinates =
      fromLat !== undefined && fromLat !== null && fromLat !== '' &&
      fromLon !== undefined && fromLon !== null && fromLon !== '' &&
      Number.isFinite(Number(fromLat)) && Number.isFinite(Number(fromLon));
    const hasDestinationCoordinates =
      toLat !== undefined && toLat !== null && toLat !== '' &&
      toLon !== undefined && toLon !== null && toLon !== '' &&
      Number.isFinite(Number(toLat)) && Number.isFinite(Number(toLon));

    // Coordinates are authoritative when the UI already selected/pinned a place.
    // Resolve start and destination independently so a missing destination pin
    // can never force a valid manual start back through text geocoding.
    const start=hasStartCoordinates
