...
// Local icon layer: keeps the prototype independent of third-party icon runtime exports.
type LocalIconProps = { size?: number|string; className?: string; style?: React.CSSProperties; title?: string; strokeWidth?: number; [key:string]: any };
const makeIcon = (name:string, glyph:string) => {
  const Icon = ({size=20, className='', style, title}:LocalIconProps) => <span className={`sarthi-icon ${className}`} title={title} aria-hidden={title ? undefined : true} style={{width:size,height:size,lineHeight:1,...style}}>{glyph}</span>;
  Icon.displayName=name;
  return Icon;
};
const ArrowLeft=makeIcon('ArrowLeft','←'), ArrowRight=makeIcon('ArrowRight','→'), Bell=makeIcon('Bell','♢'),
CalendarDays=makeIcon('CalendarDays','▦'), Check=makeIcon('Check','✓'), ChevronRight=makeIcon('ChevronRight','›'),
CircleCheck=makeIcon('CircleCheck','✓'), CircleHelp=makeIcon('CircleHelp','?'), Compass=makeIcon('Compass','⌾'),
ExternalLink=makeIcon('ExternalLink','↗'), Footprints=makeIcon('Footprints','⌁'), Globe2=makeIcon('Globe2','◎'),
HeartHandshake=makeIcon('HeartHandshake','♡'), Hospital=makeIcon('Hospital','✚'), HomeIcon=makeIcon('Home','⌂'),
Info=makeIcon('Info','i'), LocateFixed=makeIcon('LocateFixed','⊙'), MapPin=makeIcon('MapPin','●'),
Navigation=makeIcon('Navigation','➤'), Phone=makeIcon('Phone','☎'), RefreshCw=makeIcon('RefreshCw','↻'),
RouteIcon=makeIcon('Route','⌁'), Search=makeIcon('Search','⌕'), SettingsIcon=makeIcon('Settings','⚙'),
Share2=makeIcon('Share2','↗'), Shield=makeIcon('Shield','◇'), Star=makeIcon('Star','★'), TrainFront=makeIcon('TrainFront','▣'),
User=makeIcon('User','●'), Users=makeIcon('Users','●●'), WalletCards=makeIcon('WalletCards','▤'), Wifi=makeIcon('Wifi','⌁'),
WifiOff=makeIcon('WifiOff','×'), X=makeIcon('X','×'), Zap=makeIcon('Zap','ϟ'), Clock3=makeIcon('Clock3','◷'),
AlertTriangle=makeIcon('AlertTriangle','!'), Building2=makeIcon('Building2','▥'), Car=makeIcon('Car','▰'), Bus=makeIcon('Bus','▣'),
Bike=makeIcon('Bike','◌'), Moon=makeIcon('Moon','◐'), Sun=makeIcon('Sun','☀'), Play=makeIcon('Play','▶'),
MessageSquare=makeIcon('MessageSquare','▢'), CloudRain=makeIcon('CloudRain','☁'), Smartphone=makeIcon('Smartphone','▯'),
MapIcon=makeIcon('Map','▤'), Flag=makeIcon('Flag','⚑'), Send=makeIcon('Send','➤'), LogOut=makeIcon('LogOut','↪'),
Sparkles=makeIcon('Sparkles','✦'), Plus=makeIcon('Plus','+');

// Defensive icon wrapper: if a deployment ever resolves a Lucide export as missing/invalid,
// the app renders a neutral fallback instead of crashing the entire React tree.
type SafeIconProps = { icon?: any; size?: number|string } & React.HTMLAttributes<HTMLSpanElement>;
function SafeIcon({icon: Icon, size=20, ...props}: SafeIconProps){
  const Candidate = typeof Icon === 'function' ? Icon : makeIcon('FallbackIcon','•');
  try { return <Candidate size={size} {...props}/>; } catch { return <span aria-hidden="true" className="sarthi-icon" style={{width:size,height:size}}>•</span>; }
}
...
function InfoBtn({text}:{text:string}){const [o,setO]=useState(false);return <span className="info-wrap"><button className="info-btn" onClick={e=>{e.stopPropagation();setO(!o)}} title="Why I’m asking"><SafeIcon icon={Info} size={14}/></button>{o&&<span className="info-pop"><b>Why I’m asking</b>{text}</span>}</span>}
...
function MapView({center,path,markers,live=false,onMarkerClick}:{center:Coords;path?:Coords[];markers?:{coords:Coords;label:string;kind:'checkpoint'|'help'}[];live?:boolean;onMarkerClick?:(m:{coords:Coords;label:string;kind:'checkpoint'|'help'})=>void}){
 ...
 return <div className={`map-view${mapFailed?' map-view-fallback':''}`} ref={ref}>{mapFailed&&<div className="map-fallback-message"><SafeIcon icon={MapPin} size={18}/><span>Map preview is temporarily unavailable. Your journey flow is still available.</span></div>}</div>
}
...
export default function App(){
 ...
 const sendPreparedSms=(kind:'normal'|'awareness'|'eta'|'support'|'tight',reason='')=>{const recipients=trustedRecipients(kind==='tight'||kind==='support');if(!recipients.length){toastMsg('No trusted contacts are configured for SMS handoff.');return}const body=(smsDraft?.kind===kind&&smsDraft.body)||prepareSms(kind,reason);window.location.href=`sms:${recipients.join(',')}?body=${encodeURIComponent(body)}`};
 const scheduleCheckin=(reason:string,delay=1200)=>{if(checkinTimerRef.current!=null)window.clearTimeout(checkinTimerRef.current);checkinTimerRef.current=window.setTimeout(()=>showCheckinFor(reason),delay)};
 ...
 const PublicBack=()=> <button className="public-back" onClick={goBack}><SafeIcon icon={ArrowLeft} size={15}/> Back</button>;
 ...
 useEffect(() => {
   let cancelled=false;
   const check=async()=>{
     const result=typeof checkSarthiConnectivity==='function'?await checkSarthiConnectivity():{state:'Offline' as const,latency:null};
     if(cancelled)return;
     ...
     if(journeyActive&&previous!==result.state){
       ...
       if(result.state==='Offline'){setJourneyLevel(1);setBuzz('light');prepareSms('awareness','Connectivity became unavailable during an active journey');toastMsg('Connectivity changed. This is a context signal, not automatically an emergency.');scheduleCheckin('Connectivity became unavailable during an active journey',1500);}
     }
 ...
 useEffect(()=>{if(!journeyActive||!expectedArrivalAt)return;const checkEta=()=>{if(screen==='live'&&Date.now()>expectedArrivalAt&&journeyLevel<2&&!showCheckin){setJourneyLevel(1);setBuzz('light');prepareSms('awareness','Expected arrival time missed; journey completion is not confirmed');logJourneyEvent('ETA_MISSED',{expectedArrivalAt});scheduleCheckin('Expected arrival time missed; journey completion is not confirmed');}};checkEta();const id=window.setInterval(checkEta,10000);return()=>window.clearInterval(id)},[journeyActive,expectedArrivalAt,journeyLevel,showCheckin,screen]);
...
const finishProfile=()=>{if(editingProfile){const p={...profileDraft,completed:true,phone};setProfile(p);write('sarthi-profile',p);setEditingProfile(false);toastMsg('Profile updated. Your usual travel style is saved.');goBack()}else{const p={...profileDraft,completed:false,phone,priorities:[]};setProfile(p);setPriorities([]);write('sarthi-profile',p);nav('onboarding-preferences')}};
 ...
 const fetchJourneyPoints=async(geometry:[number,number][],seedPoints:JourneyPoint[]=[] )=>{
   ...
   const qParts=sample.flatMap(([lat,lng])=>[
     ...
     `nwr(around:700,${lat},${lng})["tourism"~"attraction|museum|hotel"];`,
 ...
 useEffect(()=>{if(!journeyActive||!selected||!expectedArrivalAt)return;const active=read<any>('sarthi-journey-capsule',null);if(!active|| (active.routeId===selected.id && Number(active.routeDuration)===Number(selected.duration) && active.route===selected.title))return;const oldEta=expectedArrivalAt;const nextEta=Date.now()+Math.max(1,Number(selected.duration)||1)*60000;const reason='Selected route was changed or recalculated.';etaChangeFromRef.current=oldEta;etaNewRef.current=nextEta;setExpectedArrivalAt(nextEta);setEtaUpdateReason(reason);write('sarthi-journey-capsule',{...active,routeId:selected.id,routeDuration:selected.duration,route:selected.title,expectedArrivalAt:nextEta});prepareSms('eta',reason);logJourneyEvent('ETA_UPDATED',{oldEtaAt:oldEta,newEtaAt:nextEta,reason});toastMsg('Expected arrival updated because your selected route changed.');},[selected?.id,journeyActive]);
 ...
 const renderStartPoint=()=> <div className="app-page">
  ...
  <div className="card choices">
    <button className={from==='Current location'?'selected':''} onClick={useCurrent}>
      <SafeIcon icon={LocateFixed}/><div><b>Use current location</b><span>{location?locationText:'Ask for location access'}</span></div><SafeIcon icon={Check}/>
    </button>
    <div className="search-inline">
      <SafeIcon icon={Search}/><input placeholder="Search an address, place or landmark" value={startSearch} onChange={e=>searchStartPlaces(e.target.value)}/>
      {searchingStart&&<span className="spinner"/>}
    </div>
    {startSearchError&&<div className="points-empty"><SafeIcon icon={MapPin}/><span>{startSearchError}</span></div>}
    {startResults.length>0&&<div className="start-search-results">
      ...
      }}><SafeIcon icon={MapPin}/><span>{p.display_name}</span><SafeIcon icon={ChevronRight}/></button>)}
    </div>}
    {from&&<div className="selected-place-card">
      <div><span className="eyebrow">STARTING POINT SELECTED</span><b>{from==='Current location'?locationText:from}</b><small>{from==='Current location'?'Current location is set as your start.':'Ready to use for this journey.'}</small></div><SafeIcon icon={Check}/>
    </div>}
    {!from&&<div className="points-empty"><SafeIcon icon={MapPin}/><span>Search for a starting location or choose current location.</span></div>}
  </div>
  <button className="primary pink full" disabled={!from.trim()||!fromCoords} onClick={()=>nav('destination')}>Use this starting point <SafeIcon icon={ArrowRight}/></button>
</div>;
...
const renderDestination=()=> <div className="app-page">
  ...
  <div className="search-box big">
    <SafeIcon icon={Search}/><input autoFocus placeholder="Search place, address, college, station or landmark" value={search} onChange={e=>searchPlaces(e.target.value)}/>
    {searching&&<span className="spinner"/>}
  </div>
  {search&&<div className="search-result-list">
    {results.length>0
      ? results.map((p,i)=><button key={`${p.display_name}-${i}`} onClick={()=>selectPlace(p)}>
          <SafeIcon icon={MapPin}/><div><b>{p.display_name.split(',').slice(0,2).join(', ')}</b><span>{p.display_name}</span></div><SafeIcon icon={ChevronRight}/>
        </button>)
      : !searching&&<div className="points-empty"><SafeIcon icon={MapPin}/><span>{searchError||'No matching locations were found.'}</span></div>}
  </div>}
  {!search&&<><div className="section-title">Suggested places</div><div className="suggestion-grid">
    {FALLBACK_PLACES.slice(0,6).map(x=><button key={x} onClick={()=>{setTo(x);setToCoords(null);toastMsg('Destination selected. Continue when ready.')}}><SafeIcon icon={MapPin}/><span>{x}</span></button>)}
  </div></>}
  {to&&<div className="selected-place-card destination-selected">
    <div><span className="eyebrow">DESTINATION SELECTED</span><b>{to}</b><small>{toCoords?'Location pinned from your selection.':'Selected place will be geocoded by the routing backend.'}</small></div><SafeIcon icon={Check}/>
  </div>}
  <button className="primary pink full" disabled={!to.trim()} onClick={()=>nav('mode')}>Continue to travel mode <SafeIcon icon={ArrowRight}/></button>
  <div className="map-card"><MapView center={toCoords||fromCoords||location||{lat:28.6139,lng:77.209}}/><div className="map-note"><SafeIcon icon={MapIcon} size={16}/> Map updates as you choose a location.</div></div>
</div>;

const renderMode=()=> <div className="app-page">{header('How would you like to travel?','Your usual modes are already known. You can change them for this journey.','destination')}<Bubble text="I compare practical journey options; road-route data is connected while some multimodal/provider details remain demo data."/><div className="mode-grid large"><button className={selectedMode==='Let Sarthi decide'?'selected':''} onClick={()=>setSelectedMode('Let Sarthi decide')}><SafeIcon icon={Compass}/><span>Let Sarthi decide</span></button>{MODES.map(([name,Icon])=><button className={selectedMode===name?'selected':''} key={name} onClick={()=>setSelectedMode(name)}><SafeIcon icon={Icon}/><span>{name}</span></button>)}</div>{selectedMode==='Cab'&&<div className="card cab-sync"><SafeIcon icon={Car}/><div><b>Cab provider connection</b><span>Prototype shows provider, vehicle number, model and live journey. Real deployment can connect to Uber/Ola/Rapido through an authorised provider API.</span></div><button onClick={()=>toastMsg('Cab provider sync simulated: MetroCab • DL 01 AB 1234 • Sedan')}>Sync provider</button></div>}<button className="primary turq" onClick={()=>nav('priorities')}>Continue to Journey Priorities <SafeIcon icon={ArrowRight}/></button></div>;
 const renderPriorities=()=> <div className="app-page">{header('What matters to you?','Your usual choices were collected once. Pick up to 7 for this journey style.','mode')}<Bubble text="You don’t need to rate everything. Choose the things that usually matter most to you; I’ll still consider other relevant conditions and explain them."/><div className="selected-count"><b>{priorities.length}/7</b><span>preferences selected</span></div><div className="priority-grid">{PRIORITY_OPTIONS.map(p=>{const recommendedPriority=['Faster travel','Lower walking','Fewer transfers','Help access','Connectivity','Weather comfort','Easy first / last mile'].includes(p);return <button key={p} className={priorities.includes(p)?'selected':''} onClick={()=>savePriority(p)} disabled={!priorities.includes(p)&&priorities.length>=7}><span>{p}{recommendedPriority&&<em className="recommend-badge">Recommended</em>}</span>{priorities.includes(p)&&<SafeIcon icon={Check}/>}<InfoBtn text={`I use ${p.toLowerCase()} as one input when comparing practical journeys. It never becomes a promise of safety.`}/></button>})}</div><div className="info-box"><SafeIcon icon={Info}/><span>Saved travel style can be adjusted here for this journey only. Context like weather, connectivity and help access may still matter even when they are not selected.</span></div><button className="primary pink" onClick={()=>{if(editingPreferences){const p={...profile,priorities};setProfile(p);write('sarthi-profile',p);setEditingPreferences(false);toastMsg('Journey preferences updated.');goBack()}else nav('route-analysis')}}>{editingPreferences?'Save journey preferences':'Analyze my journey'} {editingPreferences?<SafeIcon icon={Check}/>:<SafeIcon icon={ArrowRight}/>}</button></div>;
 const renderRouteAnalysis=()=> <div className="app-page centered"><div className="analysis-orb"><SafeIcon icon={Sparkles}/></div><span className="eyebrow">SARTHI CONTEXT ENGINE</span><h1>Understanding the journey around you…</h1><p>Checking route geometry, travel options, context freshness, your saved preferences and available support points.</p><div className="loading-list"><span>✓ Starting point</span><span>✓ Destination</span><span>✓ Travel style</span><span>✓ Available route context</span><span>✓ Help/checkpoint preparation</span></div><button className="primary pink" onClick={buildRoute}>{loadingRoutes?'Loading…':'Show journey options'} <SafeIcon icon={ArrowRight}/></button></div>;
...
const renderRoutes=()=> <div className="app-page">
  ...
  {!routes.length&&<div className="points-empty"><SafeIcon icon={MapPin}/><span>No route options were returned for this journey. Try the same selected locations again when the routing service is available.</span></div>}
  ...
  {selected&&<button className="primary pink" onClick={()=>nav('preparation')}>Choose {selected.title} <SafeIcon icon={ArrowRight}/></button>}
...
const renderRouteDetail=()=> selected?<div className="app-page">
  ...
  <button className="primary pink" onClick={()=>nav('preparation')}>Choose this journey <SafeIcon icon={ArrowRight}/></button>
</div>:null;

const renderCompare=()=> <div className="app-page">{header('Compare journeys','See the trade-offs before choosing.','routes')}<div className="compare-table"><div className="compare-head"><span></span>{routes.map(r=><b key={r.id}>{r.title}</b>)}</div>{[['Time',...routes.map(r=>`${r.duration} min`)],['Cost',...routes.map(r=>`₹${r.cost}`)],['Walking',...routes.map(r=>`${r.walking} min`)],['Transfers',...routes.map(r=>String(r.transfers))],['Modes',...routes.map(r=>r.modes.join(' + '))],['Confidence',...routes.map(r=>r.confidence)],['Freshness',...routes.map(r=>r.updated)]].map(row=><div className="compare-row" key={String(row[0])}>{row.map((x,i)=><span key={`${row[0]}-${i}`}>{String(x)}</span>)}</div>)}</div><div className="card"><b>What Sarthi is doing</b><p>It compares practical options using your travel style and available context. “Journey Fit” is a ranking concept, not a safety percentage.</p></div><button className="primary pink" onClick={()=>nav('preparation')}>Continue with {selected?.title||'selected route'} <SafeIcon icon={ArrowRight}/></button></div>;
 const renderPreparation=()=> selected?<div className="app-page">{header('Prepare before you leave','I’ll prepare the essentials now so the journey can continue even if your battery or internet falls.','routes')}<Bubble text="At Start Journey I create your Journey Capsule: route, checkpoints, help points, destination and emergency essentials."/><div className="prepare-hero"><SafeIcon icon={CircleCheck}/><h2>Journey ready</h2><p>{selected.title} • {selected.duration} min • {selected.modes.join(' + ')}</p><div className="eta-card"><SafeIcon icon={Clock3}/><div><span className="eyebrow">EXPECTED ARRIVAL</span><b>Calculated when you start</b><small>Based on the selected route duration — no live-traffic claim.</small></div></div></div><div className="capsule-grid"><div><SafeIcon icon={RouteIcon}/><b>Full route</b><span>Prepared</span></div><div><SafeIcon icon={Flag}/><b>Checkpoints</b><span>Prepared</span></div><div><SafeIcon icon={HeartHandshake}/><b>Help points</b><span>Prepared</span></div><div><SafeIcon icon={Shield}/><b>Emergency</b><span>Ready</span></div></div><JourneyPointsPanel/><div className="card"><div className="section-title">Journey Capsule contents <InfoBtn text="This information is stored locally as the essential fallback when the network is unstable or unavailable."/></div><ul className="plain-list"><li>Complete essential route + steps</li><li>All important checkpoints and distances</li><li>Nearby help points</li><li>Destination + last sync</li><li>Emergency contacts + helpline</li><li>Cab/provider details when available</li></ul></div><button className="primary pink" onClick={startJourney}>START JOURNEY <SafeIcon icon={Navigation}/></button></div>:null;
 const renderLive=()=> selected?<div className="app-page live"><div className="live-header"><div><span className="live-chip">● LIVE JOURNEY</span><h1>{to||'Your destination'}</h1><span>{selected.title} • {selected.duration} min estimate</span></div><button className="disha-mini" onClick={()=>nav('journey-health')}><Disha size={40}/></button></div><MapView center={liveCenter} path={path} markers={routeMarkers} live/><div className="live-status-row"><span><SafeIcon icon={LocateFixed}/> {liveCenter.lat.toFixed(4)}, {liveCenter.lng.toFixed(4)}</span><span><BatteryIcon v={battery ?? 0}/>{battery == null ? 'Battery unavailable' : `${battery}%`}</span><span><SafeIcon icon={Wifi} size={15}/> {connectivity}</span></div><div className="eta-live-card"><div><span className="eyebrow">EXPECTED ARRIVAL</span><b>{formatEta(expectedArrivalAt)}</b><small>{journeyLevel===1?'Journey context changed — completion is not yet confirmed.':etaUpdateReason||'Based on the selected route duration.'}</small></div><SafeIcon icon={Clock3}/></div>{journeyResumedNotice&&<div className="card journey-resumed"><SafeIcon icon={Check}/><div><b>Journey Resumed</b><span>You confirmed you’re okay. Emergency support remains accessible throughout the active journey.</span></div><button onClick={()=>setJourneyResumedNotice(false)}><SafeIcon icon={X}/></button></div>}{smsDraft&&<div className="card sms-ready-card"><div><span className="eyebrow">JOURNEY MESSAGE READY</span><b>{smsDraft.kind==='eta'?'ETA update':smsDraft.kind==='awareness'?'Journey update':'Journey started'}</b><small>Review the prepared contextual message before using your phone’s SMS handoff.</small></div><button className="secondary" onClick={()=>sendPreparedSms(smsDraft.kind,smsDraft.kind==='awareness'?'Journey context changed':etaUpdateReason)}><SafeIcon icon={Send}/> Send SMS</button></div>}<div className="step-card"><div className="current-step"><span>NOW</span><b>{selected.steps[journeyStep]||selected.steps[0]}</b></div><div className="current-step next"><span>NEXT</span><b>{selected.steps[Math.min(journeyStep+1,selected.steps.length-1)]}</b></div><div className="progress-rail">{selected.steps.map((_,i)=><i className={i<=journeyStep?'done':''} key={i}/>)}</div><button className="secondary full" onClick={async()=>{const nextStep=Math.min(journeyStep+1,selected.steps.length-1);setJourneyStep(nextStep);await logJourneyEvent('CHECKPOINT_REACHED',{checkpoint:selected.steps[nextStep]||`Step ${nextStep+1}`});toastMsg('Journey checkpoint recorded.')}}><SafeIcon icon={Check}/> Step completed</button></div>{((battery!=null&&battery<=40)||connectivity!=='Normal')&&<div className="health-alert"><div><b>{battery!=null&&battery<=5?'Critical Low Battery':battery!=null&&battery<=20?'Battery getting low':battery!=null&&battery<=40?'Low-Power Journey Mode':'Connectivity changed'}</b><span>{battery!=null&&battery<=40?'Essential journey data is already prepared. ':''}{connectivity!=='Normal'?'Offline-ready mode is available.':''}</span></div><button onClick={()=>nav('journey-health')}>View</button></div>}<div className="journey-level-strip"><span>LEVEL {journeyLevel}</span><b>{journeyLevel===0?'Normal':journeyLevel===1?'Awareness':journeyLevel===2?'Check-in':journeyLevel===3?'Support':'Escalation'}</b><small>Unexpected journey events are context signals, not automatically emergencies.</small></div><div className="live-actions"><button onClick={()=>sendPreparedSms('normal','Journey started')}><SafeIcon icon={Share2}/><span>Send SMS</span></button><button onClick={()=>nav('changed')}><SafeIcon icon={AlertTriangle}/><span>Something Changed</span></button><button onClick={()=>nav('offline')}><SafeIcon icon={WifiOff}/><span>Offline</span></button><button onClick={simulateUnexpectedStop}><SafeIcon icon={Clock3}/><span>{stopState==='moving'?'Simulate stop':stopState==='checking'?'Stop checking':'UEM escalated'}</span></button><button className="danger" onClick={()=>nav('emergency')}><SafeIcon icon={Shield}/><span>Emergency</span></button><button onClick={()=>nav('report')}><SafeIcon icon={CircleHelp}/><span>Report</span></button></div><JourneyPointsPanel compact/><div className="checkpoint-bar"><div><SafeIcon icon={Flag}/><b>Next checkpoint</b><span>{nextCheckpoint?.name||'No mapped checkpoint yet'}</span></div><div><SafeIcon icon={HeartHandshake}/><b>Nearby help point</b><span>{nearestHelp?.name||'Live lookup pending'}</span></div></div><button className="primary navy full" onClick={()=>nav('complete')}>I’ve arrived <SafeIcon icon={CircleCheck}/></button></div>:null;
 ...
 const st=sampleSteps[sampleStepNo];const demoPath=path.length>1?path:[{lat:28.6139,lng:77.209},{lat:28.625,lng:77.218},{lat:28.638,lng:77.226}];const demoMarkers=[{coords:demoPath[Math.max(1,Math.floor(demoPath.length*.35))],label:'Checkpoint A',kind:'checkpoint' as const},{coords:demoPath[Math.max(1,Math.floor(demoPath.length*.65))],label:'Checkpoint B',kind:'checkpoint' as const},{coords:demoPath[Math.max(1,Math.floor(demoPath.length*.5))],label:'Help point (demo)',kind:'help' as const}];const triggerStep=(n:number)=>{setSampleStepNo(n);if(n>=16)setBuzz('light');if(n>=17)setBuzz('tight');if(n<16)setBuzz('none')};return <div className="public sample"><PublicBack/><div className="tour-top"><Logo/><span>COMPLETE SAMPLE JOURNEY</span></div><div className="sample-content"><Disha size={96}/><span className="eyebrow">26-STEP MENTOR DEMO</span><h1>A real journey, from entry to support.</h1><p className="lead">Use Next to walk through the complete product story. Nothing skips the story or silently returns to the dashboard.</p><div className="sample-stage"><div className="sample-bar"><span className="live-chip">{st[0]} • {st[1]}</span><span>{st[2]}</span><BatteryIcon v={battery ?? 0}/><span>{battery == null ? 'Battery unavailable' : `${battery}%`}</span></div><MapView center={liveCenter} path={demoPath} markers={demoMarkers} live/><div className="sample-current"><span className="eyebrow">WHAT THE JUDGE SHOULD SEE</span><b>{st[2]}</b><p>{st[3]}</p></div><div className="sample-events">{sampleSteps.slice(Math.max(0,sampleStepNo-2),Math.min(sampleSteps.length,sampleStepNo+3)).map(([num,title])=><button className={num===st[0]?'done':''} key={num} onClick={()=>triggerStep(sampleSteps.findIndex(x=>x[0]===num))}><span>{num}</span>{title}<SafeIcon icon={ChevronRight}/></button>)}</div></div><div className="sample-controls"><button className="secondary" onClick={()=>sampleStepNo>0&&triggerStep(sampleStepNo-1)} disabled={sampleStepNo===0}><SafeIcon icon={ArrowLeft}/> Back</button><button className="primary pink" onClick={()=>sampleStepNo<sampleSteps.length-1&&triggerStep(sampleStepNo+1)} disabled={sampleStepNo===sampleSteps.length-1}>Next <SafeIcon icon={ArrowRight}/></button><button className="secondary" onClick={exitFlow}>Exit</button></div><div className="sample-jump"><span>Jump to key moment</span><button onClick={()=>triggerStep(6)}>Routes</button><button onClick={()=>triggerStep(12)}>Offline</button><button onClick={()=>triggerStep(17)}>UEM</button><button onClick={()=>triggerStep(23)}>Reports</button>{buzz==='tight'&&<button className="sample-uem-off" onClick={deactivateUem}><SafeIcon icon={Shield} size={13}/> Deactivate UEM</button>}</div></div></div>};
 const renderCheckinOverlay=()=>showCheckin?<div className="modal-backdrop"><div className="checkin-modal"><div className="checkin-icon"><SafeIcon icon={CircleHelp}/></div><span className="eyebrow">LEVEL 2 • CHECK-IN</span><h2>ARE YOU OKAY?</h2><p>We noticed a change in your journey context. This does not automatically mean an emergency.</p><div className="checkin-context"><div><SafeIcon icon={MapPin}/><span><b>Last location</b>{journeyLocationLabel(livePos||fromCoords||location)}</span></div><div><BatteryIcon v={battery??0}/><span><b>Battery</b>{battery==null?'Unavailable':`${battery}%`}</span></div><div><SafeIcon icon={Wifi}/><span><b>Connectivity</b>{connectivity}</span></div><div><SafeIcon icon={Clock3}/><span><b>Last update</b>{lastSync}</span></div></div><div className="checkin-actions"><button className="primary turq" onClick={handleCheckinYes}><SafeIcon icon={Check}/> YES, I'M OKAY</button><button className="secondary" onClick={handleCheckinNo}><SafeIcon icon={HeartHandshake}/> NO, I NEED HELP</button></div><button className="text-btn checkin-emergency" onClick={()=>{setShowCheckin(false);setJourneyLevel(0);nav('emergency')}}><SafeIcon icon={Shield}/> Emergency support</button></div></div>:null;
 const renderSupportOverlay=()=>journeyLevel===3?<div className="modal-backdrop"><div className="support-modal"><div className="support-head"><div><span className="eyebrow">LEVEL 3 • SUPPORT</span><h2>What’s happening?</h2></div><button onClick={()=>{setJourneyLevel(2);setShowCheckin(true)}}><SafeIcon icon={X}/></button></div>{supportPanelStep==='reason'&&<><p>Choose the closest description. This helps Sarthi prepare the right contextual support message.</p><div className="support-reasons">{['Route stopped','Internet unavailable','Battery low','Lost / I don’t know my location','I feel unsafe','Other'].map(r=><button key={r} className={supportReason===r?'selected':''} onClick={()=>completeSupportReason(r)}>{r}<SafeIcon icon={ChevronRight}/></button>)}</div></>}{supportPanelStep==='location'&&<><p>Where are you?</p><div className="support-location-choice"><button className={supportLocationChoice==='current'?'selected':''} onClick={()=>chooseSupportLocation('current')}><SafeIcon icon={LocateFixed}/><b>USE MY CURRENT LOCATION</b><span>{livePos?journeyLocationLabel(livePos):'Browser location will be requested if available.'}</span></button><button className={supportLocationChoice==='last'?'selected':''} onClick={()=>chooseSupportLocation('last')}><SafeIcon icon={MapPin}/><b>USE LAST KNOWN LOCATION</b><span>{journeyLocationLabel(livePos||fromCoords||location)}</span></button></div></>}{supportPanelStep==='options'&&<><div className="support-summary"><b>{supportReason||'Help requested'}</b><span>{supportLocationChoice==='current'?'Current location':'Last-known location'} • {journeyLocationLabel(livePos||fromCoords||location)}</span></div><div className="support-options"><button onClick={()=>sendPreparedSms('support',supportReason||'Traveller requested help')}><SafeIcon icon={Send}/><b>Send SMS</b><span>Prepared with journey context; opens the phone SMS handoff.</span></button><button onClick={shareLocation}><SafeIcon icon={Share2}/><b>Share Location</b><span>Uses current location when available, otherwise last-known.</span></button><button onClick={()=>{setJourneyLevel(0);nav('contacts')}}><SafeIcon icon={Users}/><b>Trusted Contacts</b><span>Review configured contacts.</span></button><button onClick={()=>{setJourneyLevel(0);nav('get-safety')}}><SafeIcon icon={Navigation}/><b>Get to Safety</b><span>Find nearby mapped support points.</span></button><button className="red" onClick={()=>{window.location.href='tel:112'}}><SafeIcon icon={Shield}/><b>Emergency / 112</b><span>Call emergency services when needed.</span></button></div></>}</div></div>:null;
 ...
 return <div className="app-shell"><div className="phone-frame"><div className="app-content">{renderBody()}</div>{renderCheckinOverlay()}{renderSupportOverlay()}{!publicScreens&&profile.completed&&<><button className="float-disha" onClick={()=>toastMsg('Disha: I’m here to explain any part of the journey.') }><Disha size={54}/></button><div className="bottom-nav"><button className={screen==='home'?'active':''} onClick={()=>nav('home')}><SafeIcon icon={HomeIcon}/><span>Home</span></button><button className={['journey-type','start-point','destination','mode','priorities','route-analysis','routes','route-detail','compare','preparation','live','journey-health','offline','changed','report','review','complete'].includes(screen)?'active':''} onClick={()=>nav('journey-type')}><SafeIcon icon={RouteIcon}/><span>Journeys</span></button><button className={['emergency','get-safety','contacts','uem'].includes(screen)?'active emergency':''} onClick={()=>nav('emergency')}><SafeIcon icon={Shield}/><span>Emergency</span></button><button className={screen==='settings'?'active':''} onClick={()=>nav('settings')}><SafeIcon icon={SettingsIcon}/><span>Settings</span></button></div></>}{toast&&<div className="toast"><SafeIcon icon={Sparkles} size={15}/>{toast}</div>}{buzz==='light'&&<div className="buzz light-buzz"><SafeIcon icon={Bell}/><div><b>LIGHT BUZZ SENT</b><span>Early-awareness alert prepared for primary contacts. SMS delivery requires device handoff/provider.</span></div><button onClick={()=>setBuzz('none')}><SafeIcon icon={X}/></button></div>}{buzz==='tight'&&screen!=='uem'&&<div className="buzz tight-buzz"><SafeIcon icon={Bell}/><div><b>URGENT — TIGHT BUZZ</b><span>UEM is active. Urgent journey alert prepared for every configured contact. SMS delivery requires device handoff/provider.</span></div><button onClick={()=>nav('uem')}><SafeIcon icon={Shield}/></button></div>}</div></div>;
}
