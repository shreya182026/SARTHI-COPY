...
export default function App(){
 ...
 const [toast,setToast]=useState(''); const [reportType,setReportType]=useState('Incorrect route information'); const [reportText,setReportText]=useState(''); const [reportStatus,setReportStatus]=useState<'Under Review'|'Verified'|'Questionable'|'Outdated'>('Under Review'); const evidenceInputRef=useRef<HTMLInputElement|null>(null); const [evidencePhoto,setEvidencePhoto]=useState('');
 ...
 const [feedback,setFeedback]=useState(''); const [profileDraft,setProfileDraft]=useState(profile); const [editingProfile,setEditingProfile]=useState(false); const [editingPreferences,setEditingPreferences]=useState(false); const [replay,setReplay]=useState<any|null>(null);
...
const buildRoute = async () => {
  ...
  try {
    ...
    const scoredRoutes: Array<{route: Route; score: number}> = [];
    ...
    for (
      ...
      try {
        // Use the existing logistic-regression model endpoint so the
        // route cards are driven by the model rather than the older
        // hand-weighted suitability formula.
        const priorityMatch =
          priorities.length === 0
            ? 0.5
            : Math.min(
                1,
                (
                  (priorities.includes('Faster travel') ? 0.2 : 0) +
                  (priorities.includes('Lower walking') ? (walking <= 10 ? 0.2 : 0) : 0) +
                  (priorities.includes('Fewer transfers') ? (transfers <= 1 ? 0.2 : 0) : 0) +
                  (priorities.includes('Lower cost') ? (route.estimatedCost <= 100 ? 0.2 : 0) : 0) +
                  (priorities.includes('Help access') ? 0.1 : 0) +
                  (priorities.includes('Connectivity') ? 0.1 : 0)
                )
              );
        ...
        const suitRes = await fetch(
          ...
          `&connectivity=${encodeURIComponent(connectivity)}` +
          `&battery=${battery}` +
          `&helpPoints=${transitStations.length > 0 ? 3 : 2}` +
          `&rain=${w.rain ?? 0}` +
        ...
        // ML suitability is optional; route generation still works if
        // the prediction endpoint is temporarily unavailable.
      }
      ...
      const routeRecord: Route = {
        ...
        context:
          ...
          (
            hasTransit
              ? ' • Nearby public-transit infrastructure found (not live arrival data)'
        ...
        fingerprint: [
          ...(priorities.includes('Faster travel') ? ['time'] : []),
          ...(priorities.includes('Lower walking') ? ['walking'] : []),
          ...(priorities.includes('Fewer transfers') ? ['fewer transfers'] : []),
          ...(priorities.includes('Lower cost') ? ['cost'] : []),
          ...(priorities.includes('Connectivity') ? ['connectivity'] : []),
          ...(priorities.includes('Help access') ? ['help access'] : []),
          ...(priorities.includes('Weather comfort') ? ['weather comfort'] : []),
          ...(priorities.includes('Easy first / last mile') ? ['first/last mile'] : []),
        ].slice(0, 4)
      };
      scoredRoutes.push({route: routeRecord, score: Number(suit?.suitabilityScore ?? 0.5)});
    }

    scoredRoutes.sort((a,b)=>b.score-a.score);
    const ranked = scoredRoutes.map((x,index)=>({
      ...x.route,
      title: `Route ${index+1}`,
      reason: index===0
        ? 'Currently matches the available route/context information and your selected priorities.'
        : x.route.reason
    }));
    setRoutes(ranked.length ? ranked : makeFallbackRoutes());
 ...
 const startJourney=async()=>{const c=fromCoords||location||{lat:28.6139,lng:77.209};const journeyId=`journey-${Date.now()}`;const capsule={journeyId,startedAt:new Date().toISOString(),plannedDate,plannedTime,route:selected?.title||'Route 1',routeDetails:selected,from,to,fromCoords,toCoords,mode:selected?.modes||[selectedMode],geometry:selected?.geometry||[],steps:selected?.steps||[],checkpoints:visiblePoints.filter(m=>m.kind==='checkpoint'),helpPoints:visiblePoints.filter(m=>m.kind==='help'),destination:to,contacts,helpline,stateName,lastSync,pointUpdatedAt};setJourneyActive(true);setLivePos(c);setBattery(74);setConnectivity(navigator.onLine?'Normal':'Offline');setStopState('moving');write('sarthi-active',capsule);write('sarthi-journey-capsule',capsule);try{await fetch('/api/journey-event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({journeyId,eventType:'JOURNEY_STARTED',latitude:c.lat,longitude:c.lng,battery:74,connectivity:navigator.onLine?'Normal':'Offline',checkpoint:null,from,to,destination:to,routeId:selected?.id||null})})}catch{}nav('live')};
 ...
 const submitReport=async()=>{
  ...
  const report={reportId:`report-${Date.now()}`,journeyId:active?.journeyId||'active-journey',type:reportType,description:reportText,evidencePhoto,createdAt:new Date().toISOString(),latitude:livePos?.lat??null,longitude:livePos?.lng??null,status:'Under Review'};
  const reports=read<any[]>('sarthi-reports',[]);
  reports.unshift(report); write('sarthi-reports',reports);
  try{await fetch('/api/report-event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(report)})}catch{}
  setReportStatus('Under Review'); nav('review');
};
 const renderReport=()=> <div className="app-page">{header('Report a journey issue','Keep reporting connected to the journey itself.','live')}<Bubble text="A report can help correct route/context information. Add evidence when available; the system should not automatically trust every report."/><div className="card form"><label>What needs attention?</label><select value={reportType} onChange={e=>setReportType(e.target.value)}><option>Incorrect route information</option><option>Outdated information</option><option>Incorrect help point</option><option>Changed transport condition</option><option>Incorrect journey context</option><option>Issue still unresolved</option></select><label>Describe it</label><textarea value={reportText} onChange={e=>setReportText(e.target.value)} placeholder="Tell us what changed..."/><label>Evidence</label><input ref={evidenceInputRef} type="file" accept="image/*" hidden onChange={e=>setEvidencePhoto(e.target.files?.[0]?.name||'')}/><div className="evidence-grid"><button onClick={()=>evidenceInputRef.current?.click()}>{evidencePhoto?'Photo added':'Add photo'}</button><button onClick={()=>toastMsg('Timestamp attached to this report.')}>Timestamp</button><button onClick={()=>toastMsg('Location context attached to this report.')}>Location context</button></div>{evidencePhoto&&<small className="evidence-name">Attached: {evidencePhoto}</small>}</div><button className="primary pink" onClick={submitReport}>Submit for review <Send/></button></div>;
