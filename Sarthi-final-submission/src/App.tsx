...
import {
  ...
  Map, Flag, Send, LogOut, Sparkles, Plus
} from 'lucide-react';
...
type Route={id:string;title:string;duration:number;distance:number;cost:number;walking:number;transfers:number;modes:string[];reason:string;context:string;confidence:'High'|'Moderate'|'Limited';updated:string;firstMile:string;lastMile:string;steps:string[];geometry:[number,number][];traffic:'Light'|'Moderate'|'Heavy'|'Unavailable';fingerprint?:string[];factors?:string[];helpPointCount?:number;transitAvailable?:boolean};
...
export default function App(){
 ...
 const [from,setFrom]=useState(''); const [fromCoords,setFromCoords]=useState<Coords|null>(null); const [to,setTo]=useState(''); const [toCoords,setToCoords]=useState<Coords|null>(null);
 ...
 const translatingDomRef=useRef(false);
 useEffect(()=>{
   const originals=translatedNodesRef.current;
   const attrOriginals=translatedAttrsRef.current;
   let observer:MutationObserver|undefined;
   const restore=()=>{
     originals.forEach((value,node)=>{if(node.nodeType===Node.TEXT_NODE&&node.textContent!==value)node.textContent=value;});
     attrOriginals.forEach((attrs,el)=>Object.entries(attrs).forEach(([attr,value])=>{if(el.getAttribute(attr)!==value)el.setAttribute(attr,value);}));
   };
   const translate=()=>{
     if(language!=='hi'||translatingDomRef.current)return;
     translatingDomRef.current=true;
     // DOM translation mutates text nodes. Pause observation while translating so
     // the MutationObserver cannot recursively translate its own changes.
     observer?.disconnect();
     try{
       ...
       while((node=walker.nextNode())){
         ...
         if(translated!==value&&node.textContent!==translated)node.textContent=translated;
       }
       document.querySelectorAll<HTMLElement>('input[placeholder],button[title],button[aria-label],[title]').forEach(el=>{
         (['placeholder','title','aria-label'] as const).forEach(attr=>{
           ...
           if(value){
             ...
             if(attrs[attr]===undefined)attrs[attr]=value;
       ...
       translatingDomRef.current=false;
       if(observer&&language==='hi')observer.observe(document.body,{subtree:true,childList:true,characterData:true});
     }
   };
   restore();
   observer=new MutationObserver(()=>translate());
   translate();
   if(language==='hi')observer.observe(document.body,{subtree:true,childList:true,characterData:true});
...
const buildRoute = async () => {
  if (!from.trim() || !to.trim()) {
    toastMsg('Please select both a starting point and a destination.');
    return;
  }
  ...
  try {
    ...
    if (fromCoords) {
      ...
      params.set('fromLon', String(fromCoords.lng));
    }
    if (toCoords) {
      params.set('toLat', String(toCoords.lat));
      params.set('toLon', String(toCoords.lng));
    }
    ...
    const routeStart: Coords = {
      lat: Number(routeJson.from?.lat ?? fromCoords?.lat),
      lng: Number(routeJson.from?.lon ?? fromCoords?.lng)
    };
    const routeEnd: Coords = {
      lat: Number(routeJson.to?.lat ?? toCoords?.lat),
      lng: Number(routeJson.to?.lon ?? toCoords?.lng)
    };
    ...
    const raw = routeJson.routes.filter((candidate:any)=>Array.isArray(candidate?.geometry?.coordinates)&&candidate.geometry.coordinates.length>1&&Number.isFinite(Number(candidate.durationMin))&&Number.isFinite(Number(candidate.distanceKm))).slice(0, 3);
    const rts: Route[] = [];
    ...
    for (let index = 0; index < raw.length; index++) {
      ...
      rts.push({
        ...
        steps: (route.steps || [])
          .slice(0, 8)
          .map((x: any) => x.name || x.maneuver?.instruction || 'Continue')
          .filter(Boolean),
...
const startJourney=async()=>{if(!selected||!fromCoords){toastMsg('Please select a starting location before starting the journey.');return}const c=fromCoords;const journeyId=`journey-${Date.now()}`;const capsule={journeyId,startedAt:new Date().toISOString(),route:selected?.title||'Route 1',from,to,mode:selected?.modes||[selectedMode],geometry:selected?.geometry||[],steps:selected?.steps||[],checkpoints:visiblePoints.filter(m=>m.kind==='checkpoint'),helpPoints:visiblePoints.filter(m=>m.kind==='help'),destination:to,contacts,helpline,stateName,lastSync};setJourneyActive(true);setLivePos(c);setStopState('moving');write('sarthi-active',capsule);write('sarthi-journey-capsule',capsule);try{await fetch('/api/journey-event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({journeyId,eventType:'JOURNEY_STARTED',latitude:c.lat,longitude:c.lng,battery,connectivity,checkpoint:null})})}catch{}nav('live')};
  ...
  <div className="card choices">
    ...
    {startResults.length>0&&<div className="start-search-results">
      {startResults.map((p,i)=><button key={`${p.display_name}-${i}`} onClick={()=>{
        ...
        setFrom(p.display_name.split(',').slice(0,3).join(', '));setFromCoords({lat,lng});setStartSearch('');setStartResults([]);setStartSearchError('');toastMsg('Starting point selected.');
...
const renderDestination=()=> <div className="app-page">
  ...
  <div className="map-card"><MapView center={toCoords||fromCoords||location||{lat:28.6139,lng:77.209}}/><div className="map-note"><Map size={16}/> Map updates as you choose a location.</div></div>
</div>;
