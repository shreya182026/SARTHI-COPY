...
export default function App(){
 ...
 useEffect(()=>{
   ...
   const restore=()=>{
     originals.forEach((value,node)=>{if(node.nodeType===Node.TEXT_NODE)node.textContent=value;});
     attrOriginals.forEach((attrs,el)=>Object.entries(attrs).forEach(([attr,value])=>el.setAttribute(attr,value)));
   };
   const translate=()=>{
     ...
     document.querySelectorAll<HTMLElement>('input[placeholder],button[title],button[aria-label],[title]').forEach(el=>{
       (['placeholder','title','aria-label'] as const).forEach(attr=>{
         ...
         if(value){
           ...
           if(translated!==attrs[attr])el.setAttribute(attr,translated);
         }
       });
     });
   ...
   restore();
   translate();
   const observer=new MutationObserver(()=>{if(!translatingDomRef.current)translate()});
